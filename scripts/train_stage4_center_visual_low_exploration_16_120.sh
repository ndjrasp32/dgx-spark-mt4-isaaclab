...
if [[ ! -s "${BEST_FILE}" ]]; then
  echo "[ERROR] Stage-4 visual training needs a starting checkpoint."
...
export MT4_REACH_REPLAY_PROB="${MT4_REACH_REPLAY_PROB:-0.92}"
export MT4_REACH_REPLAY_JOINT_NOISE="${MT4_REACH_REPLAY_JOINT_NOISE:-0.004}"
export MT4_REACH_REPLAY_TARGET_NOISE="${MT4_REACH_REPLAY_TARGET_NOISE:-0.001}"
export MT4_REACH_REPLAY_JOINT_VELOCITY_SCALE="${MT4_REACH_REPLAY_JOINT_VELOCITY_SCALE:-0.08}"
export MT4_REACH_FINAL_CENTER_RADIUS="${MT4_REACH_FINAL_CENTER_RADIUS:-0.035}"
export MT4_REACH_ACTION_SCALE="${MT4_REACH_ACTION_SCALE:-0.030}"
export MT4_REACH_ACTION_PENALTY="${MT4_REACH_ACTION_PENALTY:-0.018}"

echo "[INFO] Stage-4 low-exploration visual training for ${TASK_NAME}"
...
MAX_ITERATIONS="${MT4_MAX_ITERATIONS:-40}"
VIDEO_INTERVAL="${MT4_VIDEO_INTERVAL:-100000}"
VIDEO_LENGTH="${MT4_VIDEO_LENGTH:-180}"

echo "[INFO] num_envs=16 max_iterations=${MAX_ITERATIONS} video=true"
echo "[INFO] video_length=${VIDEO_LENGTH} video_interval=${VIDEO_INTERVAL}"
...
./isaaclab.sh -p scripts/reinforcement_learning/rsl_rl/train.py \
  ...
  --seed "${MT4_SEED:-42}" \
  agent.policy.init_noise_std="${MT4_INIT_NOISE_STD:-0.45}" \
  agent.algorithm.entropy_coef="${MT4_ENTROPY_COEF:-0.003}" \
  "$@"
