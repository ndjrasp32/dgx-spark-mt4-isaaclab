...
STATE_FILE="${MT4_REACH_PREGRASP_STATE_FILE:-${PROJECT_DIR}/data/pregrasp_states/latest.pt}"
...
if [[ ! -s "${BEST_FILE}" ]]; then
  echo "[ERROR] Replay-reset Stage-B training needs a starting checkpoint."
  echo "        best_checkpoint.txt was not found or is empty:"
  echo "        ${BEST_FILE}"
  echo "        Run a baseline first, then run ${PROJECT_DIR}/scripts/plot_and_select_best.sh."
  exit 1
fi

if [[ ! -f "${STATE_FILE}" ]]; then
  echo "[ERROR] Pregrasp replay state file does not exist:"
  echo "        ${STATE_FILE}"
  echo "        Run ${PROJECT_DIR}/scripts/collect_pregrasp_states.sh first."
  exit 1
fi
...
export MT4_REACH_REPLAY_PROB="${MT4_REACH_REPLAY_PROB:-0.75}"
export MT4_REACH_REPLAY_JOINT_NOISE="${MT4_REACH_REPLAY_JOINT_NOISE:-0.025}"
export MT4_REACH_REPLAY_TARGET_NOISE="${MT4_REACH_REPLAY_TARGET_NOISE:-0.004}"

echo "[INFO] Stage-B replay-reset curriculum training for ${TASK_NAME}"
